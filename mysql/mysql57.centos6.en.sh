...
#     ./mysql57.centos6.en.sh $1 $2
#     ./mysql57.centos6.en.sh init 5.7.25
...
init(){
    ...
    yum -y install libaio wget bzip2 numactl
    ...
    tar -xf mysql-$1-linux-glibc2.12-x86_64.tar.gz -C $base_dir/usr/local
    ...
    echo "expire_logs_days                                       = 30"                                   >> $config_file
    ...
    echo "port                                                   = 3306"                                 >> $config_file
    echo "query_cache_limit                                      = 2M"                                   >> $config_file
    echo "query_cache_size                                       = 32M"                                  >> $config_file
    echo "query_cache_type                                       = 2"                                    >> $config_file
    ...
    echo "sql_mode                                               = \"NO_ENGINE_SUBSTITUTION,NO_AUTO_CREATE_USER,STRICT_TRANS_TABLES\"" >> $config_file
    ...
    sudo -u mysql ./bin/mysqld --defaults-file=$config_file --initialize-insecure --user=mysql --datadir=$base_dir/usr/local/mysql/data/
    sudo -u mysql ./bin/mysql_ssl_rsa_setup --uid=$(id -u mysql) --datadir=$base_dir/usr/local/mysql/data/
