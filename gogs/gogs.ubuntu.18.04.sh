...
#     ./gogs.ubuntu.18.04.sh $1 $2 $3
#     ./gogs.ubuntu.18.04.sh init 0.11.86 0.0.0.0
...
init(){
    ...
    bind_ip="$2"
    setenforce 0
    apt -y install git wget curl openssh-server
    ...
    cd $base_dir;sudo su - git -c "$base_dir/usr/local/gogs/gogs web >> $base_dir/var/log/gogs/start.log 2>&1 &"
    sleep 3
